...
//
#ifndef DMOMINT_BASE58_H
#define DMOMINT_BASE58_H
...
/** base58-encoded DMOmint addresses.
 ...
 */
class CDMOmintAddress;
class CDMOmintAddressVisitor : public boost::static_visitor<bool>
{
private:
    CDMOmintAddress *addr;
public:
    CDMOmintAddressVisitor(CDMOmintAddress *addrIn) : addr(addrIn) { }
    bool operator()(const CKeyID &id) const;
    bool operator()(const CScriptID &id) const;
    bool operator()(const CNoDestination &no) const;
};

class CDMOmintAddress : public CBase58Data
{
public:
    enum
    {
    ...
    {
        return boost::apply_visitor(CDMOmintAddressVisitor(this), dest);
    }
    ...
    CDMOmintAddress(const CTxDestination &dest)
    {
        Set(dest);
    }

    CDMOmintAddress(const std::string& strAddress)
    {
        SetString(strAddress);
    }

    CDMOmintAddress(const char* pszAddress)
    {
...
bool inline CDMOmintAddressVisitor::operator()(const CKeyID &id) const         { return addr->Set(id); }
bool inline CDMOmintAddressVisitor::operator()(const CScriptID &id) const      { return addr->Set(id); }
bool inline CDMOmintAddressVisitor::operator()(const CNoDestination &id) const { return false; }

/** A base58-encoded secret key */
class CDMOmintSecret : public CBase58Data
{
public:
    ...
    CDMOmintSecret(const CSecret& vchSecret)
    {
        SetSecret(vchSecret);
    }
