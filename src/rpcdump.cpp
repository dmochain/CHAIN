...
Value importkey(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw runtime_error(
            "importkey <dmomintkey> [label] [rescan=true]\n"
    ...
    CKeyID vchAddress = key.GetPubKey().GetID();
    {
        ...
        if(vArgs.size() > 2) {
            ...
            if (!pwalletMain->AddPubKeyPos(CdmomintAddress(vchAddress).ToString(), pos))
...
Value dumpkey(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "dumpkey <dmomintaddress>\n"
            "Reveals the private key|public key|position corresponding to <dmomintaddress>, this would cost a few minutes");

    string strAddress = params[0].get_str();
    CdmomintAddress address;
    ...
    if (pwalletMain->GetPubKeyPos(strAddress, pos))
        return CdmomintSecret(vchSecret).ToString()+"|" + EncodeBase58(pubKey.vchPubKey) + "|" + HexStr(pos.ToVector());
    else
        return CdmomintSecret(vchSecret).ToString()+"|" + EncodeBase58(pubKey.vchPubKey);
