Dmo// Copyright (c) 2018 The Dmomint developers
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain) {
            ...
            return error("DmomintMiner : generated block is stale");
        }
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DmomintMiner : ProcessBlock, block not accepted");
    }
...
void static DmomintMiner(CWallet *pwallet)
{
    printf("DmomintMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("dmomint-miner");
    ...
    try { while(true) {
        ...
        printf("Running DmomintMiner with %" PRIszu " transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("DmomintMiner terminated\n");
        throw;
    }
}

void GenerateDmomints(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&DmomintMiner, pwallet));
}
