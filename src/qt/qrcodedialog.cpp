...
{
    QString ret = QString("dmomint:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non dmo unit input in lnReqAmount, we generate the URI with dmo as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(dmomintUnits::format(dmomintUnits::dmo, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
