...
const int DMOMINT_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString DMOMINT_IPC_PREFIX("DMOmint:");
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    {
        if (!args[i].startsWith(DMOMINT_IPC_PREFIX, Qt::CaseInsensitive))
            continue;
    ...
    foreach (const QString& arg, savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(DMOMINT_IPC_CONNECT_TIMEOUT))
            return false;
        ...
        socket->waitForBytesWritten(DMOMINT_IPC_CONNECT_TIMEOUT);
...
{
    // Install global event filter to catch QFileOpenEvents on the mac (sent when you click DMOmint: links)
    ...
    if (!uriServer->listen(name))
        qDebug() << tr("Cannot start DMOmint: click-to-pay handler");
    else
...
{
    // clicking on DMOmint: URLs creates FileOpen events on the Mac:
